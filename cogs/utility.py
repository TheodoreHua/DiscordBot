...
class Utility(commands.Cog):
    ...
    async def embed(self, ctx, *, args: TypedFlags):
        ...
        know what they're doing. If you would like the fields to not be inline, add `<!>` to the front and back of the
        field name. One very common one is colour, it has to be in decimal form, not hex. Some image arguments only work
        if their text is defined, for example `footer_icon_url` only works if `footer_text` is defined."""
        await ctx.message.delete()
